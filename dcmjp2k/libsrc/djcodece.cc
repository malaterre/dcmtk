...
#include "dcmtk/dcmjp2k/djcodece.h"
...
#include "dcmtk/dcmjp2k/djrparam.h"  /* for class D2RepresentationParameter */
...
E_TransferSyntax DJ2KNearLosslessEncoder::supportedTransferSyntax() const
{
    ...
    OFBool& removeOldRep) const
{
  ...
  DJ2KRepresentationParameter defRep;
  ...
  const DJ2KRepresentationParameter *djrp = OFreinterpret_cast(const DJ2KRepresentationParameter *, toRepParam);
...
OFCondition DJ2KEncoderBase::adjustOverlays(
  DcmItem *dataset,
    ...
    double& compressionRatio) const
{
  ...
  if (result.good())
  {
    ...
    {
      // compress frame
      DCMJ2K_DEBUG("JPEG-LS encoder processes frame " << (i+1) << " of " << frameCount);
...
// static helper functions for DJ2KEncoderBase::setCustomParameters().
...
void DJ2KEncoderBase::setCustomParameters(
#if 0
  ...
  const DJ2KCodecParameter *djcp) const
{
  ...
  {
    case DJ2KCodecParameter::interleaveSample:
      jls_params.ilv = ILV_SAMPLE;
      break;
    case DJ2KCodecParameter::interleaveLine:
      jls_params.ilv = ILV_LINE;
      break;
#ifdef ENABLE_DCMJ2K_INTERLEAVE_NONE
    case DJ2KCodecParameter::interleaveNone:
      jls_params.ilv = ILV_NONE;
      break;
#endif
    case DJ2KCodecParameter::interleaveDefault:
    default:
  ...
  {
    DCMJ2K_DEBUG("Converting image from " << (ilv == ILV_NONE ? "color-by-plane" : "color-by-pixel")
  ...
  if (result.good())
  {
    ...
    J2K_ERROR err = JpegLsEncode(&buffer, &size, &bytesWritten, framePointer, frameSize, &jls_params);
...
OFCondition DJ2KEncoderBase::losslessCookedEncode(
    const Uint16 *pixelData,
    const Uint32 length,
    DcmItem *dataset,
    const DJ2KRepresentationParameter *djrp,
    ...
    Uint16 nearLosslessDeviation) const
{
      ...
      photometricInterpretation != "YBR_FULL")
  {
    // a photometric interpretation that we don't handle. Fall back to raw encoder (unless in near-lossless mode)
     if (nearLosslessDeviation > 0) return EC_J2KUnsupportedPhotometricInterpretation;
  ...
  if (pixelRepresentation > 0)
  {
    ...
    if (nearLosslessDeviation > 0) return EC_J2KUnsupportedPixelRepresentation;
    ...
    if (photometricInterpretation == "RGB" || photometricInterpretation == "YBR_FULL")
    {
        ...
        else return EC_J2KUnsupportedPixelRepresentation;
    }
  }
  ...
  if (bitsStored > 16)
  {
    DCMJ2K_WARN("Cannot compress image with " << bitsStored << " bits/sample: JPEG-LS supports max. 16 bits.");
    return EC_J2KUnsupportedBitDepth;
  }
  if (bitsStored < 2)
  {
    DCMJ2K_WARN("Cannot compress image with " << bitsStored << " bit/sample: JPEG-LS requires at least 2 bits.");
    return EC_J2KUnsupportedBitDepth;
  }
...
OFCondition DJ2KEncoderBase::compressCookedFrame(
  DcmPixelSequence *pixelSequence,
  DicomImage *dimage,
  const OFString& /* photometricInterpretation */,
  DcmOffsetList &offsetList,
  ...
  Uint16 nearLosslessDeviation) const
{
  ...
  {
    DCMJ2K_DEBUG("Converting image from color-by-pixel to color-by-plane");
  ...
  J2K_ERROR err = JpegLsEncode(&compressed_buffer, &compressed_buffer_size, &bytesWritten, framePointer, buffer_size, &jls_params);
  result = DJ2KError::convert(err);
...
OFCondition DJ2KEncoderBase::convertToUninterleaved(
    Uint8 *target,
    const Uint8 *source,
    Uint16 components,
    Uint32 width,
...
OFCondition DJ2KEncoderBase::convertToSampleInterleaved(
    Uint8 *target,
    const Uint8 *source,
    Uint16 components,
    Uint32 width,
...
void DJ2KEncoderBase::fixPaddingIfNecessary(
    Uint8 *buffer,
    size_t bufSize,
