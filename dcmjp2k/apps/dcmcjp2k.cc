...
#include "dcmtk/dcmjp2k/dj2kutil.h"   /* for dcmjp2k typedefs */
#include "dcmtk/dcmjp2k/djencode.h"   /* for class DJ2KEncoderRegistration */
#include "dcmtk/dcmjp2k/djrparam.h"   /* for class DJ2KRepresentationParameter */
...
int main(int argc, char *argv[])
{
  ...
  DJ2KCodecParameter::interleaveMode opt_interleaveMode = DJ2KCodecParameter::interleaveLine;
...
LICENSE_FILE_EVALUATE_COMMAND_LINE_OPTIONS
      ...
      {
        opt_interleaveMode = DJ2KCodecParameter::interleaveDefault;
      }
      if (cmd.findOption("--interleave-sample"))
      {
        opt_interleaveMode = DJ2KCodecParameter::interleaveSample;
      }
      if (cmd.findOption("--interleave-line"))
      {
        opt_interleaveMode = DJ2KCodecParameter::interleaveLine;
      }
#ifdef ENABLE_DCMJPLS_INTERLEAVE_NONE
      if (cmd.findOption("--interleave-none"))
      {
        opt_interleaveMode = DJ2KCodecParameter::interleaveNone;
      }
#endif
      cmd.endOptionBlock();
    ...
    DJ2KRepresentationParameter rp(OFstatic_cast(Uint16, opt_nearlossless_deviation), opt_useLosslessProcess);
